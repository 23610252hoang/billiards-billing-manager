...
これは正式なアンケートではなく、試用後のヒアリングベースの内容です。

一方で、最初の画面では操作ボタンが多く、初めて使う人には少し分かりにくいという課題もありました。  
そのため、公開用のStreamlit版では、最初に利用中テーブル数、本日の会計数、本日の売上を表示し、初めて見る人でも目的が伝わりやすい構成にしました。

正確な秒数計測は行っていないため、「会計時間が何%減った」といった定量値は主張していません。  
ただし、店主への確認では、手計算と比べて会計確認の負担は軽くなったと分かりました。
...
旧版は、単一店舗・単一PCでの利用を想定したローカルアプリです。  
小規模な店舗では導入しやすい一方、複数端末・複数店舗で使うには設計を変える必要があります。

## 9. Future Architecture

もし複数店舗や複数端末で使う場合は、以下のようなWeb/API構成に発展させたいです。
...
この構成にする場合、以下が重要になると考えています。

- 同じテーブルを複数端末から同時に会計しないための排他制御
- 店舗ごとのデータ分離
- スタッフ、店長、管理者ごとの権限管理
- 会計変更履歴を残す監査ログ
- 障害時に復旧できるバックアップ
...
試用後、店主からは「手計算の確認が楽になる」「テーブル状況を一覧で見られるのは助かる」という趣旨のフィードバックをもらいました。  
一方で、最初は画面の操作が少し分かりにくいという課題もあったため、公開用のStreamlit版では、利用中テーブル数、本日の会計数、本日の売上を最初に表示するようにしました。
